...
class MNIST_truncated(data.Dataset):
    ...
    def __build_truncated_dataset__(self):
        ...
        # if self.train:
        #     datasets = mnist_dataobj.train_data
        #     target = mnist_dataobj.train_labels
        # else:
        #     datasets = mnist_dataobj.test_data
...
class SVHN_custom(data.Dataset):
    ...
    def __build_truncated_dataset__(self):
        if self.train is True:
            ...
            # datasets = np.concatenate((svhn_dataobj1.datasets, svhn_dataobj2.datasets), axis=0)
        ...
        if self.dataidxs is not None:
            data = data[self.dataidxs]
            target = target[self.dataidxs]
        # print("svhn datasets:", datasets)
        # print("len svhn datasets:", len(datasets))
        # print("type svhn datasets:", type(datasets))
    ...
    #         gs_index = index[i]
    #         self.datasets[gs_index, :, :, 1] = 0.0
    #         self.datasets[gs_index, :, :, 2] = 0.0
...
class CelebA_custom(VisionDataset):
    ...
    Args:
        ...
        split (string): One of {'train', 'valid', 'test', 'all'}.
            Accordingly datasets is selected.
        target_type (string or list, optional): Type of target to use, ``attr``, ``identity``, ``bbox``,
            or ``landmarks``. Can also be a list to output a tuple with all specified target types.
            The targets represent:
                ``attr`` (np.array shape=(40,) dtype=int): binary (0, 1) labels for attributes
                ``identity`` (int): label for each person (datasets points with the same identity are the same person)
...
class FEMNIST(MNIST):
    """
    This datasets is derived from the Leaf repository
    (https://github.com/TalwalkarLab/leaf) pre-processing of the Extended MNIST
    datasets, grouping examples by writer. Details about Leaf were published in
    ...
    def download(self):
        """Download the FEMNIST datasets if it doesn't exist in processed_folder already."""
        import shutil
...
class Generated(MNIST):
    ...
    def __init__(self, root, dataidxs=None, train=True, transform=None, target_transform=None,
        ...
        if self.train:
            self.data = np.load("datasets/generated/X_train.npy")
            self.targets = np.load("datasets/generated/y_train.npy")
        else:
            self.data = np.load("datasets/generated/X_test.npy")
            self.targets = np.load("datasets/generated/y_test.npy")
